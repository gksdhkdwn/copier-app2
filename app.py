...
# 1. 페이지 기본 설정import streamlit as st
import re
import urllib.parse
import json
import os
...
DEFAULT_TEMPLATES = {
    ...
    "v_multi_greeting": "안녕하세요 퍼스트 전산입니다.\n세금계산서 발행을 위해 보유하신 총 {total}대 기기의 사용량 확인 카운터 사진이 필요하여 연락드렸습니다.\n각 기기별 카운터 한장씩 보내주시면 감사하겠습니다.",
...
# ============================================================
    ...
    if raw_text and raw_text.strip():
        ...
        # [요구사항] S, NN, N급 탭이 먼저 나오도록 배치 순서 변경
        ...
        with tab_s:
            ...
            if not s_keys: st.caption("감지된 S, NN, N급 업체가 없습니다.")
                ...
                for g_idx, gkey in enumerate(s_keys):
                    ...
                    with col_target:
                        if st.button(f"🟢 {display_name} ({len(machines)}대)", key=f"btn_s_{gkey}"):
        ...
        with tab_v:
            ...
            if not v_keys: st.caption("감지된 V, SS급 업체가 없습니다.")
                ...
                for g_idx, gkey in enumerate(v_keys):
                    ...
                    with col_target:
                        if st.button(f"💎 {display_name} ({len(machines)}대)", key=f"btn_v_{gkey}"):
                            show_send_popup(display_name, phones, msg, original_names)
st.set_page_config(page_title="퍼스트전산 마감 도우미", page_icon="📱", layout="wide")
