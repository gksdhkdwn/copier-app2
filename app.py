...
if "custom_formats" not in st.session_state:
    # 이 부분이 if문 안쪽으로 들여쓰기 되어 있어야 합니다.
    st.session_state.custom_formats = {
        ...
        "D460": txt_sindo, "D470": txt_sindo, "MA2100": txt_ecosys, "M5526": txt_ecosys,
        "M5521": txt_ecosys, "ECOSYS": txt_ecosys, "305": txt_305, "5473": txt_5473,
        ...
        "5700": txt_5700, "L5100": txt_l5100, "2554": txt_ricoh, "C3003": txt_ricoh,
        "C4504": txt_ricoh, "5005": txt_5005, "X3220NR": txt_x3220, "X-9201": txt_x3220,
        "SL-": txt_samsung, "기본 기종": txt_default
    }

# 4. 탭 구성
tabs = st.tabs(["📝 문자 생성", "⚙️ 사전"])

with tabs[0]:
    raw_text = st.text_area("카톡 내용 붙여넣기:", height=200)
    ...
    if raw_text:
        ...
        for idx, block in enumerate(valid_blocks):
            # 전화번호 찾기
            phone_match = re.search(r'01[0-9]-?\d{3,4}-?\d{4}', block)
            phone_num = phone_match.group().replace("-", "") if phone_match else ""
            
            # 기종 매칭
            msg_body = st.session_state.custom_formats["기본 기종"]
            for k, v in st.session_state.custom_formats.items():
                if k.upper() in block.upper():
                    msg_body = v
                    break
