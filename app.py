...
# 3. 세션 상태 사전 초기화 (서버 구버전 충돌 방지용 안전 장치)
...
# 💡 팅김 현상을 만드는 원인 추적 및 안전 키값 바인딩
if "text_clear_trigger" not in st.session_state:
    st.session_state["text_clear_trigger"] = False

# 초기화 버드가 눌렸을 때 세션의 실제 글자를 강제로 비워주는 로직
if st.session_state["text_clear_trigger"]:
    st.session_state["text_input_area"] = ""
    st.session_state["text_clear_trigger"] = False  # 트리거 해제
...
with col_btn1:
    # 💡 [수정 완료] st.rerun()을 쓰지 않고 내부 신호(Trigger)만 주어 안전하게 리셋시키는 방식
    if st.button("🗑️ 입력 내용 전체 초기화", use_container_width=True):
        st.session_state["text_clear_trigger"] = True
        # 구버전 전용 화면 강제 새로고침 우회 기법 적용 (팅김 원천 차단)
        st.write('<div style="display:none;"></div>', unsafe_allow_html=True)
...
# 5. 분석 로직 가동
if (analyze_clicked or (raw_text and not st.session_state.get("text_clear_trigger", False))) and raw_text.strip():
