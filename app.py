...
# 💡 [핵심 수정] 핸드폰에서 손가락이 갇히지 않도록 내부 스크롤을 원천 차단하고 높이를 완전 자동 확장하는 CSS
st.markdown(
    """
    <style>
    /* 입력창 내부의 스크롤바를 숨기고 최대 높이 제한을 해제 */
    div[data-testid="stTextArea"] textarea {
        overflow-y: hidden !important; /* 내부 스크롤 절대 방지 */
        height: auto !important;
        min-height: 200px !important;
        max-height: none !important;  /* 높이 제한 해제하여 아래로 완전히 늘어남 */
    }
    </style>
    """,
...
with tabs[0]:
    # CSS 제어로 스크롤바가 원천 차단된 입력 영역
    raw_text = st.text_area("카톡 내용 붙여넣기:", value=st.session_state.input_text, key="text_input_area")
