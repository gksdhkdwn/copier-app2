...
# 3. 세션 상태 독립 초기화 (잘림 방지를 위해 한 줄로 콤팩트하게 압축!)
    ...
    if st.session_state.analyze_clicked and raw_text.strip():
        ...
        for i, block in enumerate(valid_blocks, 1):
            with st.container():
                ...
                with col1: u_name = st.text_input(f"업체명 ({i})", value=detected_name, key=f"nm_{i}_{i}")
                with col2: u_phone = st.text_input(f"연락처 ({i})", value=detected_phone, key=f"ph_{i}_{i}")
                with col3:
                    d_idx = machine_options.index(matched_machine) if matched_machine in machine_options else machine_options.index("기본 기종")
                    u_machine = st.selectbox(f"기종 ({i})", options=machine_options, index=d_idx, key=f"mc_{i}_{i}")
...
with tabs[1]:
    ...
    for machine_type in list(st.session_state.custom_formats.keys()):
        method_text = st.session_state.custom_formats[machine_type]
        col_k, col_v = st.columns([1, 3])
        with col_k: st.markdown(f"**{machine_type}**")
        with col_v: st.session_state.custom_formats[machine_type] = st.text_area(f"'{machine_type}' 설정", value=method_text, key=f"set_{machine_type}", height=80)
