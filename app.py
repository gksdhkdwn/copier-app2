            ...
            if not s_keys: 
                ...
                for g_idx, gkey in enumerate(s_keys):
                    ...
                    with btn_cols_s[g_idx % 4]:
                        if st.button(f"🟢 {display_name} ({len(machines)}대)", key=f"btn_s_{gkey}_{g_idx}", use_container_width=True):
            ...
            if not v_keys:
                ...
                for g_idx, gkey in enumerate(v_keys):
                    ...
                    with btn_cols_v[g_idx % 4]:
                        if st.button(f"💎 {display_name} ({len(machines)}대)", key=f"btn_v_{gkey}_{g_idx}", use_container_width=True):
                            show_send_popup(display_name, phones, generated_msg, original_names)
