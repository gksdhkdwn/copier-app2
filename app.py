...
st.caption("카톡 내용을 복사해 넣으면 번호가 적힌 거래처를 인식하여 그룹별 맞춤 마감 문자를 대량 생성합니다.")
...
# 2. 세션 상태(st.session_state) 사전 초기화 (팅김 및 초기화 버그 원천 차단)
if "init_done" not in st.session_state:
    st.session_state["init_done"] = True
    
    # 각 지역 그룹별 기본 문자 양식 설정
...
# 3. [설정 영역] 환경설정창 (저장 버튼 클릭 시 세션 반영 및 화면 유지)
with st.expander("⚙️ 지역별(A,B,C,D포함) 문자 양식 및 기종 설정", expanded=True):
    st.subheader("📋 지역 그룹별 문자 양식 설정")
    
    col_t1, col_t2 = st.columns(2)
    with col_t1:
        v_ss_input = st.text_area("💎 V급 / SS급 지역 문자 양식", value=st.session_state["msg_template_v_ss"])
        a_input = st.text_area("🅰️ A지역 문자 양식 ({total} 포함 가능)", value=st.session_state["msg_template_a"])
        c_input = st.text_area("🅲 C지역 문자 양식 ({total} 포함 가능)", value=st.session_state["msg_template_c"])
    with col_t2:
        s_nn_n_input = st.text_area("⭐ S / NN / N 지역 문자 양식 ({total} 포함)", value=st.session_state["msg_template_s_nn_n"])
        b_input = st.text_area("🅱️ B지역 문자 양식 ({total} 포함 가능)", value=st.session_state["msg_template_b"])
        d_input = st.text_area("🅳 D지역 문자 양식 ({total} 포함 가능)", value=st.session_state["msg_template_d"])
    ...
    if st.button("💾 설정 내용 안전하게 저장하기", type="primary", use_container_width=True):
        ...
        st.success("✅ A, B, C, D 지역 양식이 성공적으로 저장되었습니다!")
...
if raw_text and raw_text.strip():
            ...
            if cur_phones:
                ...
                if st.button(f"💬 {cur_name}{phone_count_suffix} 발송", key=f"popup_btn_{i}", use_container_width=True):
                    ...
                    def send_dialog(name=cur_name, phones=cur_phones, msg=cur_msg):
                        st.warning("⚠️ 아래 전송할 번호와 문구를 확인하신 후 최하단 링크를 눌러주세요.")
                        
                        selected_phone = phones[0]
                        if len(phones) > 1:
                            selected_phone = st.radio("📞 발송할 번호를 선택하세요:", options=phones, key=f"sel_ph_{i}")
                        else:
    ...
    # 7. 하단 상세 정보 편집 및 개별 문구 확인
    st.markdown("---")
    st.subheader("🔍 상세 정보 편집 및 지역/개별 문구 확인")
    ...
    for s_info in sms_data_list:
        ...
        with st.container():
            ...
            with col4:
                # 💡 개별 거래처 관리 박스에 신규 지역 리스트 연동
                d_reg_idx = reg_options.index(st.session_state[f"final_region_{i}"])
                u_region = st.selectbox(f"지역 등급 ({i})", options=reg_options, index=d_reg_idx, key=f"rg_{i}_f")

            # 선택된 등급에 따른 개별 미리보기 출력 로직
            total_devices = max(1, len(re.findall(r'(기종|대수|\b[A-Z0-9-]{3,}\b)', s_info["block_raw"])))
            if u_region == "V급 / SS급":
